...
# Sub2API 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/thornywetsui/sub2api-releases/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="thornywetsui/sub2api-releases"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/sub2api" ]; then
        ...
        "$INSTALL_DIR/sub2api" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+(-upstream\.[0-9]+\.[0-9]+\.[0-9]+)?' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
Documentation=https://github.com/thornywetsui/sub2api-releases
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/sub2api" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+(-upstream\.[0-9]+\.[0-9]+\.[0-9]+)?' || echo "unknown")
